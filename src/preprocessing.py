...
def load_survey(data_path, met_zone_codes, linking_cols, q):
    ...
    df_ind['Sexo'] = df_ind['Sexo'].astype(
    ...
    df_ind['SeguroIMSS'] = df_ind['SeguroIMSS'].astype(
        'category').cat.rename_categories(['no_imss', 'imss'])

    df_ind['SeguroPriv'] = df_ind['SeguroPriv'].astype(
        'category').cat.rename_categories(['no_privado', 'privado'])

    df_ind['ConexionInt'] = df_ind['ConexionInt'].astype(
        'category').cat.rename_categories(['internet', 'no_internet'])
    ...
    # Find the bin ranges
    df_ind['Ingreso'] = pd.qcut(df_ind['Ingreso'], q,
...
def load_census(data_path, state_code, met_zone_codes):
    data_path = Path(data_path)

    # census_path = data_path / f'RESAGEBURB_{state_code}CSV20.csv'
    census_path = data_path / f'census/RESAGEBURB_{state_code}_2020_csv.zip'
    ...
    df_censo = pd.read_csv(census_path,
                           usecols=cols, na_values=['*', 'N/D'],
                           low_memory=False)

    # Keep only aggregates by AGEB
    df_censo = df_censo[df_censo['NOM_LOC'] == 'Total AGEB urbana']
    df_censo = df_censo.reset_index(drop=True)

    # Filter by state and met zone
    df_censo = df_censo[
        (df_censo['ENTIDAD'] == state_code)
        & (df_censo['MUN'].isin(met_zone_codes))].copy()

    # Create CVEGEO column, and drop columns no longer useful
    df_censo['cvegeo'] = df_censo.apply(
        lambda x: f'{x.ENTIDAD}{x.MUN:03}{x.LOC:04}{x.AGEB}',
        axis=1)
    df_censo.drop(
    ...
    df_censo[int_cols] = df_censo[int_cols].astype(int)
...
def load_census_full(data_path, state_code, met_zone_codes):
    data_path = Path(data_path)
